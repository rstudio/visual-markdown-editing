...
| List Check      | [x]                                                        |                   |
| Definition Term | :                                                          |                   |
| Link            | `[text](href)`{.inherit-text} *or* `<href>`{.inherit-text} | Ctrl+K            |
| Image           |                                                            | Shift+Ctrl+I      |
...
By default, visual mode uses a content width and font size generally comfortable for writing (similar to that used in Google Docs). You can adjust the maximum content width (by default it's 700 pixels) as well as the font size (by default the same as the IDE) within the **R Markdown -\> Advanced** preferences.
