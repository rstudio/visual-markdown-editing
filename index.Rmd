...
Visual markdown editing is currently only available in the daily build of RStudio (v1.4.286 or later). You can download the most recent daily build here: <https://dailies.rstudio.com>.
...
If you have set the `unsafe` option in your Hugo `config.toml` to allow inclusion of raw html, you'll want to let the visual editor know that this is a supported extension:

    <!-- -*- mode: goldmark; extensions: +raw_html; doctype: blogdown -*- -->

If you are using a version of Hugo earlier than v0.60, then the default markdown engine is `blackfriday`, so you'll need to use this format comment:
